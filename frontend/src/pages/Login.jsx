...
export default function Login({ onLogin }) {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      localStorage.setItem('adminName', data.adminName || data.name || 'Admin')
      onLogin()
    } catch (err) {
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{
          ...
          {/* Error message */}
          {error && (
            <div className="alert alert-danger" style={{ marginBottom: '20px', fontSize: '0.85rem' }}>
              <span>⚠️</span>
              <span>{error}</span>
            </div>
          )}
        ...
        <p style={{ textAlign: 'center', color: '#475569', fontSize: '0.78rem', marginTop: '20px' }}>
          © 2025 Paper Plane. All rights reserved.
        </p>
      </div>
    </div>
  )
